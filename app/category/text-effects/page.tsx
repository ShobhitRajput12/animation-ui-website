...
import { TEXT_EFFECTS_DATA } from './data'

export default function TextEffectsPage() {
  const animations = TEXT_EFFECTS_DATA.map(({ previewCode, code, prompt, ...rest }: any) => rest).sort((a: any, b: any) => (a.createdAt < b.createdAt ? 1 : -1))

  return (
    <div className="flex-1 overflow-y-auto flex flex-col bg-bg-primary">
      <Navbar title="Text Effects" />
      <div className="p-6">
        {animations.length === 0 ? (
